...
const vmessUUID = "eeeeeee3-eeee-4eee-aeee-eeeeeeeeeee5";
...
export default {
    async fetch(request, env, ctx) {
        try {
            ...
            // WebSocket proxy dengan strict custom path
            if ((upgradeHeader || "").toLowerCase() === "websocket") {
                const pxip = url.pathname.match(/^\/mediafairy\/(.+[:=-]\d+)$/);
                
                // Jika path sesuai dengan format /mediafairy/ip:port
                if (pxip) {
                    globalThis.pxip = pxip[1];
                    return await websocketHandler(request);
                } else {
                    // Blokir paksa semua koneksi WebSocket dengan path lain
                    return new Response("Unauthorized Path", { status: 403 });
                }
            }

            // Stealth root response
            if (url.pathname === "/" || url.pathname === "") {
                return new Response("WELCOME", { status: 200 });
            }

            // Non-WS traffic: pass-through
